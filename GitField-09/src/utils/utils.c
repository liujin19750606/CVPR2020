...
/*
int SplitStringc(char *str, char *needle, char *new_str, char **stack)
{
	strcpy(new_str, str);
	char *p = new_str;
	int count = 0;
...
int remove_needlec(char *new_str, char **stack, int n)
{
	if (n == 0)
		return 0;
	...
	char *new_str2 = malloc((max_L + 1)*sizeof(char)),
		**stack2 = malloc(256 * sizeof(char*));//new char*[256];
...
list *get_paths2(char *filelist, float *base_path, char *ext)
{
	list *lines;
	char *path;
	...
	if (file)
	{
		...
		while ((path = fgetl(file)))
		{
			...
			else
			{
				...
				sprintf(full_path, "%s/%s%s", base_path, path, ext);
				list_insert(lines, full_path);
			}
		}
		fclose(file);
		return lines;
...
list *get_paths_filter(char *filelist, char *base_path, char *ext)
{
	list *lines;
	char *path;
	FILE *file = fopen(filelist, "r");
...
list *get_paths_fname(char *filename, char *fname)
{
	list *lines;
	char *path;
	FILE *file = fopen(filename, "r");
...
float mag_array2(float *a, int n)
{
	int i;
	float sum = 0;
	for (i = 0; i < n; ++i)
...
unsigned char *read_file(char *filename, size_t *pSize)
{
